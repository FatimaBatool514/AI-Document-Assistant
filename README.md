...
make sure `requirements.txt` is in the same repository directory as `app.py`
(or the repository root used by the deployed entrypoint), and that it contains:

```text
faiss-cpu==1.15.0
```

After committing the changed `requirements.txt`, redeploy/restart the app so
Streamlit Cloud rebuilds the Python environment. Community Cloud installs
Python dependencies from `requirements.txt`, and the Python version matters
for binary packages such as FAISS.
